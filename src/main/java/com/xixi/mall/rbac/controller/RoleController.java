...
public class RoleController {
    ...
    public ServerResponse<List<RoleVO>> list() {
        UserInfoInTokenBO userInfoInTokenBO = AuthUserContext.get();
        return ServerResponse.success(roleService.list(userInfoInTokenBO.getSysType(), userInfoInTokenBO.getTenantId()));
    }

	@GetMapping
    @ApiOperation(value = "获取角色", notes = "根据roleId获取角色")
    public ServerResponse<RoleVO> getByRoleId(@RequestParam Long roleId) {
        return ServerResponse.success(roleService.getByRoleId(roleId));
    }
    ...
    public ServerResponse<Void> update(@Valid @RequestBody RoleDTO roleDTO) {
        ...
        if (!Objects.equals(dbRole.getBizType(), userInfoInTokenBO.getSysType()) || !Objects.equals(dbRole.getTenantId(), userInfoInTokenBO.getTenantId())) {
            return ServerResponse.fail(ResponseEnum.UNAUTHORIZED);
        }
        Role role = mapperFacade.map(roleDTO, Role.class);
        role.setBizType(userInfoInTokenBO.getSysType());
